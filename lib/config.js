...
const DEFAULTS = {
  ...
  yolo: true,
  allowedAadIds: [],
  voiceFile: path.join(__dirname, "..", "prompts", "teams-voice.md"),
};
...
function load() {
  ...
  // The voice prompt is what stops the agent replying with a wall of diff to
  // someone who is driving. Read it here so a missing or renamed file is a
  // startup error in the banner, not a silently ordinary-sounding agent.
  const voiceFile = path.resolve(
    (process.env.TEAMS_VOICE_FILE ?? file.voiceFile ?? DEFAULTS.voiceFile).replace(
      /^~(?=$|\/)/,
      process.env.HOME,
    ),
  );
  cfg.voiceFile = voiceFile;
  cfg.voice = fs.existsSync(voiceFile) ? fs.readFileSync(voiceFile, "utf8").trim() : null;
